const CONFIG = {
    ...
    api: {
        ...
        contact: 'https://lianxi.mrwang.com/hezuo.php'
    }
};
...
function renderWhoisDomainLink(domain) {
    ...
    return escapeHtml(normalizedDomain.toUpperCase());
}
...
function switchSponsorPay(type) {
    ...
    if (tipElement) {
        tipElement.textContent = `Scan with ${type === 'alipay' ? 'Alipay' : 'Telegram'}`;
    }
}
...
function formatDomainDuration(duration, kind) {
    ...
    const parts = [];
    const years = normalizedValue.match(/(\d+)\s*(?:Y|years?|yrs?)/i);
    const months = normalizedValue.match(/(\d+)\s*(?:Mo|months?|mos?)/i);
    const days = normalizedValue.match(/(\d+)\s*(?:D|days?)/i);

    if (years) {
        const yearCount = parseInt(years[1], 10);
        parts.push(`<b>${years[1]}</b> ${yearCount === 1 ? 'yr' : 'yrs'}`);
    }
    if (months) {
        const monthCount = parseInt(months[1], 10);
        parts.push(`<b>${months[1]}</b> ${monthCount === 1 ? 'mo' : 'mos'}`);
    }
    if (days) {
        const dayCount = parseInt(days[1], 10);
        parts.push(`<b>${days[1]}</b> ${dayCount === 1 ? 'day' : 'days'}`);
    }

    const yearIncluded = Boolean(years);
    const monthIncluded = Boolean(months);
    const dayCount = days ? parseInt(days[1], 10) : 0;
    const formattedValue = parts.join(' ');

    if (kind === 'age' && !yearIncluded && !monthIncluded && dayCount < 7) {
        return `${formattedValue} <span class="status-badge status-badge--recent">Recently registered</span>`;
    }

    if (kind === 'remaining') {
        if (isExpired) {
            return `<span class="status-badge status-badge--error">Expired ${formattedValue}</span>`;
        }

        if (!yearIncluded && !monthIncluded && dayCount < 7) {
            return `${formattedValue} <span class="status-badge status-badge--warning">Expiring soon</span>`;
        }
    }
...
async function openWhois(domain) {
    ...
    try {
        ...
        if (whoisData.unknown) {
            ...
            updateWhoisField('whois-name-servers', 'This domain cannot be recognized or is not supported for lookup yet');
            return;
        }

        if (!whoisData.registered) {
            updateWhoisField('whois-age', 'Unregistered');
            updateWhoisField('whois-remaining', whoisData.reserved ? 'Reserved' : 'Available to register');
            updateWhoisField('whois-registrar', whoisData.reserved ? 'Reserved domain' : '--');
            updateWhoisField('whois-created-at', '--');
            updateWhoisField('whois-updated-at', '--');
            updateWhoisField('whois-expires-at', '--');
            updateWhoisField(
            ...
            updateWhoisField('whois-name-servers', whoisData.reserved ? 'This domain has been reserved' : 'This domain has not been registered yet');
            return;
        }
        ...
        updateWhoisField('whois-registrar', whoisData.registrar || 'Unknown');
        ...
        updateWhoisField('whois-name-servers', 'Lookup failed');
        ...
        .map((item) => {
            ...
            return `
                    <div class="site-card">
                        ...
                        <button type="button" class="site-card__whois-button" data-whois-domain="${escapeHtml(whoisDomain)}" aria-label="Lookup ${escapeHtml(whoisDomain)} Whois information">
...
async function getPublicIp() {
    ...
    throw new Error('Public IP retrieval failed');
}
...
async function bindContactForm() {
    ...
    ipStatusElement.textContent = 'Getting network environment information...';
    try {
        currentIp = await getPublicIp();
        ipStatusElement.textContent = 'Network environment check completed. The form can be submitted.';
    } catch (error) {
        currentIp = '';
        ipStatusElement.textContent = 'Failed to retrieve network environment information. Please check your network and try again.';
    }
    ...
    formElement.addEventListener('submit', async (event) => {
        ...
        if (!currentIp) {
            setContactResult('Network environment information has not been retrieved successfully yet, so the form cannot be submitted for now.', false);
            return;
        }

        const payload = buildContactPayload(currentIp);
        if (!validateContactPayload(payload)) {
            setContactResult('Please fill in all required fields.', false);
            return;
        }
        ...
        try {
            ...
            try {
                ...
                setContactResult('Submission failed. Please try again later.', false);
                return;
            }

            if (response.ok && result.success === true) {
                setContactResult(result.message || 'Submission successful. We will contact you as soon as possible.', true);
                formElement.reset();
            } else {
                setContactResult(result.message || 'Submission failed. Please try again later.', false);
            }
        } catch (error) {
            setContactResult(`Request failed: ${error.message}`, false);
        } finally {
